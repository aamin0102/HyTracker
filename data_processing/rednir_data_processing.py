...
from band_selection_15 import band_selection
...
def image_storage(aug_image, copy_gt, training_image_count, path):
    ...
    image_path = os.path.join(path, 'rednir_{:010d}.jpg'.format(training_image_count + 1))
    cv2.imwrite(image_path, aug_image)
    text_path = os.path.join(path, 'rednir_{:010d}.txt'.format(training_image_count + 1))
...
def validation_image_storage(aug_image, copy_gt, training_image_count, path):
    ...
    image_path = os.path.join(path, 'rednir_{:05d}.jpg'.format(training_image_count + 1))
    cv2.imwrite(image_path, aug_image)
    text_path = os.path.join(path, 'rednir_{:05d}.txt'.format(training_image_count + 1))
    h, w, _ = aug_image.shape
    yolo_gt = convert_to_yolo(copy_gt, w, h)
    if os.path.isfile(text_path):
        with open(text_path, 'w') as file:
...
if __name__ == "__main__":
    ...
    training_dataset_directory = "D:\\hsi_2023_dataset\\training\\hsi\\rednir"
    training_videos = os.listdir(training_dataset_directory)
    training_videos.sort()

    validation_dataset_directory = "D:\\hsi_2023_dataset\\validation\\hsi\\rednir"
    ...
    for i in range(len(training_videos)):
        ...
        number_of_class = class_name["training_"+"rednir_"+training_videos[i]]
    ...
    for i in range(len(validation_videos)):
        ...
        number_of_class = class_name["validation_rednir_" + validation_videos[i]]
